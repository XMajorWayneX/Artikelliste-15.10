...
function ManualEntries() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(entriesCollection, (snapshot) => {
      ...
      const sortedEntries = [...entriesData].sort((a, b) => {
        if (a.status === 'angefragt' && b.status !== 'angefragt') {
          return -1; // 'angefragt' entries come first
        }
        if (a.status !== 'angefragt' && b.status === 'angefragt') {
          return 1; // 'angefragt' entries come first
        }
      ...
      checkOverdueEntries(sortedEntries);
    }, (error) => {
  ...
  const checkOverdueEntries = (entries) => {
    const hasOverdue = entries.some(entry => {
      const isOverdue = isPastDueDate(entry.abgabeBis, entry.status);
      console.log(`Entry: ${entry.projektname}, Status: ${entry.status}, AbgabeBis: ${entry.abgabeBis}, isOverdue: ${isOverdue}`);
      return isOverdue;
    });
    console.log(`hasOverdueEntries: ${hasOverdue}`);
  ...
  const isPastDueDate = (dueDateString, status) => {
    if (status === 'angebot erhalten' || status === 'Absage') return false;
    if (!dueDateString) return false;
    try {
      const dueDate = new Date(dueDateString);
      const now = new Date();
      const isOverdue = dueDate < now;
      console.log(`Status: ${status}, AbgabeBis: ${dueDateString}, DueDate: ${dueDate}, Now: ${now}, isOverdue: ${isOverdue}`);
      return isOverdue;
    } catch (error) {
      console.error("Error parsing date string:", dueDateString, error);
      return false;
    }
  };
  ...
  const angefragtVonOptions = ["Fetzer", "Baus", "Ehnert", "Weber"].sort();
  ...
  return (
    <div>
      ...
      <table className="manual-entries-table">
        ...
        <tbody>
          <tr style={getInputRowStyle()} ref={inputFormRef}>
